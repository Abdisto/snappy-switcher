...
static void add_xdg_dirs(const char **list, int *idx, const char *subdir)
{
	...
	while (dir) {
		static char path[64][PATH_MAX];	// storage for generated paths
		static int path_idx = 0;

		snprintf(path[path_idx], PATH_MAX, "%s/%s", dir, subdir);
		list[(*idx)++] = path[path_idx];
		path_idx++;
...
static void init_paths(void)
{
	...
	if (home) {
		snprintf(user_icons_path, sizeof(user_icons_path), "%s/.icons",
			 home);
		icon_dirs[icon_idx++] = user_icons_path;
	}

	if (data_home) {
		snprintf(user_icons_path2, sizeof(user_icons_path2), "%s/icons",
			 data_home);
	} else if (home) {
	...
	/* XDG system icon paths */
	add_xdg_dirs(icon_dirs, &icon_idx, "icons");
	...
	/* XDG system desktop paths */
	add_xdg_dirs(desktop_dirs, &desktop_idx, "applications");

	/* flatpak fallback */
	desktop_dirs[desktop_idx++] =
				...
				int size)
{
	...
	for (int d = 0; icon_dirs[d]; d++) {
		...
		for (size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]); s++) {
			for (size_t c = 0;
			     c < sizeof(categories) / sizeof(categories[0]);
			     c++) {
				for (size_t e = 0;
				     e <
					...
					snprintf(path, sizeof(path),
						 "%s/%s/%s/%s/%s%s",
						 icon_dirs[d], theme, sizes[s],
						 categories[c], icon_name,
						 extensions[e]);
					if (file_exists(path))
						return path;
				}
		...
		for (size_t c = 0;
		     c < sizeof(categories) / sizeof(categories[0]); c++) {
			for (size_t s = 0; s < sizeof(sizes) / sizeof(sizes[0]);
			     s++) {
				for (size_t e = 0;
				     e <
				     sizeof(extensions) / sizeof(extensions[0]);
	...
	for (size_t e = 0; e < sizeof(extensions) / sizeof(extensions[0]); e++) {
		snprintf(path, sizeof(path), "/usr/share/pixmaps/%s%s",
			 icon_name, extensions[e]);
		if (file_exists(path))
			return path;
	}
...
static char *find_desktop_file_by_scan(const char *class_name)
{
	...
	for (int d = 0; desktop_dirs[d]; d++) {
		...
		while ((entry = readdir(dir)) != NULL) {
			...
			to_lowercase(lowercase_name, name,
	...
	for (int p = 0; flatpak_patterns[p]; p++) {
		for (int d = 0; desktop_dirs[d]; d++) {
			snprintf(desktop_path, sizeof(desktop_path),
				 flatpak_patterns[p], class_name);
			char full_path[768];	/* Larger to avoid truncation */
			snprintf(full_path, sizeof(full_path), "%s/%s.desktop",
				 desktop_dirs[d], desktop_path);
			if (file_exists(full_path)) {
				char *icon =
				    extract_icon_from_desktop(full_path);
				if (icon)
					return icon;
			}
		}
...
static cairo_surface_t *find_png_fallback(const char *icon_name, int size)
{
	...
	const char *themes[] =
	    { current_theme, fallback_theme_name, "hicolor", "Adwaita" };
	...
	for (size_t t = 0; t < sizeof(themes) / sizeof(themes[0]); t++) {
		for (size_t rs = 0;
		     rs < sizeof(raster_sizes) / sizeof(raster_sizes[0]);
		     rs++) {
			for (int d = 0; icon_dirs[d]; d++) {
				if (!icon_dirs[d][0])
					continue;
				for (size_t cat = 0;
					...
					snprintf(try_path, sizeof(try_path),
						 "%s/%s/%s/%s/%s.png",
						 icon_dirs[d], themes[t],
						 raster_sizes[rs],
						 categories[cat], icon_name);
					if (file_exists(try_path)) {
						cairo_surface_t *s =
						    load_png_icon(try_path,
								  size);
						if (s) {
	...
	snprintf(try_path, sizeof(try_path), "/usr/share/pixmaps/%s.png",
		 icon_name);
	if (file_exists(try_path)) {
...
static cairo_surface_t *load_svg_icon(const char *path, int size)
{
	...
	rsvg_handle_get_intrinsic_dimensions(handle, &has_width,
					     &intrinsic_width, &has_height,
					     &intrinsic_height, &has_viewbox,
					     &intrinsic_viewbox);
	...
	if (!render_ok) {
		LOG("SVG render failed (%s): %s", path,
...
void icons_init(const char *theme_name, const char *fallback)
{
	...
	if (fallback && fallback[0]) {
		strncpy(fallback_theme_name, fallback,
			sizeof(fallback_theme_name) - 1);
		fallback_theme_name[sizeof(fallback_theme_name) - 1] = '\0';
	}

	cache_count = 0;
	LOG("Initialized: theme=%s, fallback=%s", current_theme,
	    fallback_theme_name);
}
...
cairo_surface_t *load_app_icon(const char *class_name, int size)
{
	...
	if (!icon_name) {
		...
		if (cache_count < MAX_CACHE) {
			strncpy(icon_cache[cache_count].class_name, class_name,
				127);
	...
	if (icon_name[0] == '/' && file_exists(icon_name)) {
		...
		if (ext) {
			if (strcasecmp(ext, ".png") == 0) {
				...
				if (dot)
					*dot = '\0';
				LOG("SVG found but RSVG disabled, trying PNG fallback for: %s", base_name);
		...
		if (surface) {
			/* Cache result */
			if (cache_count < MAX_CACHE) {
				strncpy(icon_cache[cache_count].class_name,
					class_name, 127);
				icon_cache[cache_count].size = size;
		...
		LOG("Direct load failed, trying theme search for: %s",
		    icon_name);
	}
	...
	if (!icon_path) {
		icon_path =
		    find_icon_in_theme(fallback_theme_name, icon_name, size);
	}
	if (!icon_path) {
		...
		if (ext) {
			if (strcasecmp(ext, ".png") == 0) {
				...
				if (!surface) {
					...
					surface =
					    find_png_fallback(icon_name, size);
				}
			}
#endif
#ifndef HAVE_RSVG
			else if (strcasecmp(ext, ".svg") == 0) {
				LOG("SVG found but RSVG disabled, searching PNG fallback for: %s", icon_name);
